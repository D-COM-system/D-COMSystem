...
//    qDebug() << dbPath;
    ...
    QSqlQuery query;
    QString createQuery = "CREATE TABLE user (username varchar(20) primary key ,password varchar(20),cusNumber varchar(20))";
    ...
    for (int row = 2; row <= sheet->dimension().lastRow(); ++row) {
        ...
        QString insertQuery = "INSERT INTO user (username, password, cusNumber) "
                              "VALUES (?, ?, ?)";
